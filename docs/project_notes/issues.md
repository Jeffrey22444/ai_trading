...
- **Notes**: Baseline backend tests passed before edits (`112 passed, 1 warning`). Current backend tests pass (`137 passed, 1 warning`). AI analysis now outputs only regime classification; deterministic code converts regime + F1-F4/Q + Direction + existing quant guardrails into entry/hold decisions and applies the regime risk gate. Execution uses bounded retries, persists position state in the existing analysis JSON, and releases capital only after the post-close position refresh confirms flat. TREND/BREAKOUT entries currently block until the missing setup selector rule is specified.
